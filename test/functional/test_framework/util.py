...
def get_fee(tx_size, feerate_dgb_kvb):
    ...
    feerate_sat_kvb = int(feerate_dgb_kvb * Decimal(1e8)) # Fee in sat/kvb as an int to avoid float precision errors
    target_fee_sat = ceildiv(feerate_sat_kvb * tx_size, 1000) # Round calculated fee up to nearest sat
    return satoshi_round(target_fee_sat / Decimal(1e8)) # Truncate DGB result to nearest sat
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "digibyte.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
